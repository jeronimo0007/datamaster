...
# Funcao para pausar entre demos
pause() {
    echo ""
...
batch = []
for t in data[:20]:  # Usar 20 para a demo ser rapida
    batch.append({
        'amount': t['amount'],
        ...
        'hour': int(t['timestamp'].split('T')[1].split(':')[0]) if 'T' in t['timestamp'] else 12,
        'is_weekend': 0,
...
echo -e "${BLUE}  Dashboard disponivel em: http://localhost:8501${NC}"
echo -e "${BLUE}  API Swagger em: http://localhost:8000/docs${NC}"
echo -e "${BLUE}============================================================${NC}"
