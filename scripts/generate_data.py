...
def generate_transaction(transaction_id: str = None) -> dict:
    """Gera uma transação simulada"""
    if not transaction_id:
        transaction_id = str(uuid.uuid4())
    
    # Simula alguns casos de fraude
    is_fraud = random.random() < 0.05  # 5% de fraudes
    ...
    return {
        ...
        'merchant_category': random.choice([
            'Eletrônicos', 'Alimentação', 'Vestuário', 
            'Serviços', 'Viagem', 'Entretenimento'
        ]),
...
def generate_batch(num_transactions: int = 100, output_file: str = None) -> list:
